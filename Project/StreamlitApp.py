...
class Detector():
    ...
    def detect_one(self, **kwargs):
        ...
        if kwargs.get('visualize', False):
            np_img = np.array(img)
            visualization.draw_dt_on_np(np_img, detections, **kwargs)
            st.image(np_img, caption='Input Image', use_column_width=True)
        return detections
...
def ProcessImageData(uploaded_file):
    # Initialize detector
    ...
    im = detector.detect_one(img_path= uploaded_file,
...
def PreProcessWifiData(_df):    
    # convert the first seen and last seen column to datetime
    _df['firstSeen'] = pd.to_datetime(_df['firstSeen'])
    _df['lastSeen'] = pd.to_datetime(_df['lastSeen']) 

    # convert the hour to a 12 hour format
    _df['firstSeen'] = _df['firstSeen'] + pd.Timedelta(hours = 12)
    _df['lastSeen'] = _df['lastSeen'] + pd.Timedelta(hours = 12)

    # insert the difference time between the start and end time into the dataframe
    _df.insert(loc=3, column='diff_time', value= _df.lastSeen - _df.firstSeen)
    
    _df = _df[_df.vendorIsLocal != True]
    return _df

def plotDayData(df):
    dft = df.firstSeen.dt.day.value_counts().to_frame()
    ...
    ax = fig.subplots()
    sns.barplot(
        y=dft["firstSeen"], x=dft["day"], color="goldenrod", ax=ax
    )
    ax.set_title("Number of entries per Day")
    ax.set_xlabel("Year")
    ax.set_ylabel("day")
...
with st.sidebar:
    add_radio = st.radio(
        ...
        ("Wifi Analysis", "Fisheye Detection")
    )

if (add_radio == "Wifi Analysis"):
    st.header("Wifi Probe analysis")
    add_radio = st.radio(
        "Plot Data",
        ("All Data", "Randomised Data", "Non Randomised Data")
    )
    file = st.file_uploader('Upload CSV file', type='csv')
    if file:  # if user uploaded file
        # read csv file
        df = pd.read_csv(file)
        df = PreProcessWifiData(df)
        # sample data
        # st.write(df.head())
        # plot data
        plotDayData(df)
        #Number of unique random and non random mac addresses
        plotRandomMacSize(df)
        
                
        
########################################################################################
if (add_radio == "Fisheye Detection"):
    st.header("Generate Heatmap using Positional Data")
    st.write("Load number of images and get corresponding positions of detected people:")
    # Image upload tab
    ...
    if uploaded_file is not None:
        ...
        im = ProcessImageData(uploaded_file)
